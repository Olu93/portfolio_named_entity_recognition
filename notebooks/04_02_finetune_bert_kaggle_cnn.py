...
# dataset = load_from_disk(DATASETS_DIR / 'ner_annotations_combined.conll')
...
def load_conll_data(file_path):
    ...
    with open(file_path, 'r', encoding='utf-8') as f:
        for idx,line in enumerate(f):
            ...
            if line == "":
                if current_sentence:
                    sent = " ".join([s[0] for s in current_sentence])
                    tags = [s[1] for s in current_sentence]
                    tokens = [s[0] for s in current_sentence]
                    sentences.append({"text": sent, "ner_tags": tags, "tokens": tokens})
                    current_sentence = []
            else:
                parts = line.split()
                if len(parts) >= 2:
...
dataset = Dataset.from_list(sentences, features=new_features)
